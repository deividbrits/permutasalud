...
export const comunasPorRegion: Record<string, string[]> = {
  ...
  "Región de Aysén del Gral. Carlos Ibáñez del Campo": ["Coyhaique", "Lago Verde", "Aysén", "Cisnes", "Guaitecas", "Cochrane", "O'Higgins", "Tortel", "Chile Chico", "Río Ibáñez"],
...
export const getRegionByComuna = (comuna: string): string | null => {
  if (!comuna) return null;
  const searchComuna = comuna.toLowerCase().trim();
  for (const [region, comunas] of Object.entries(comunasPorRegion)) {
    if (comunas.some(c => c.toLowerCase().trim() === searchComuna)) {
      return region;
    }
  }
  return null;
